...
<div class="image-container">
    <img src="./docs/javascript/images/21.png" alt="图片14-1" title="图14-1" >
...
<div class="image-container">
    <img src="./docs/javascript/images/22.png" alt="图片14-2" title="图14-2" >
...
```html
<div id="test">
  <div class="first-child">这是第一个子节点</div>
  <div class="second-child">这是第二个子节点</div>
</div>
```
...
```js
const moveLastNode = (container = document.body, node) =>
...
运行示例，第一个子节点将会被移动到 test 元素的最后一个节点，效果如下:
...
<div class="ew-code-compiler">
   <div id="test">
    ...
    <script>
        const moveLastNode = (container = document.body,node) => container.appendChild(node);
        moveLastNode(document.querySelector('#test'),document.querySelector('.first-child'));
    </script>
</div>
...
alert(deepList.childNodes.length); // 3（IE9 之前的版本）或 7（其他浏览器）,每个节点都会有一个text节点，因此是7
...
虽然 DOM 规范规定 Document 节点的子节点可以是 DocumentType、Element、ProcessingInstruction 或 Comment，但也提供了两个访问子节点的快捷式。第一个是 documentElement 属性，始终指向 HTML 页面中的`<html>`元素。虽然 document.childNodes 中始终有`<html>`元素，但使用 documentElement 属性可以更快更直接地访问该元素。假如有以下简单的页面：
...
一般来说，appendChild()、removeChild()和 replaceChild()方法不会用在 document 对象上。这是因为文档类型（如果存在）是只读的，而且只能有一个 Element 类型的子节点（<em class="hover-info" data-title="元素是 HTMLHtmlElement 的实例，HTMLHtmlElement 继承 HTMLElementHTMLElement 继承 Element，因此 HTML文档可以包含子节点，但不能多于一个。">即`<html>`，已经存在了</em>）。
...
URL 跟域名是相关的。比如，如果 document.URL 是 `http://www.wrox.com/WileyCDA/`，则 document.domain 就是www.wrox.com。
...
- nodeType 等于 1；  nodeName 值为元素的标签名；
- nodeValue 值为 null；
...
如前所述，所有 HTML 元素都是 HTMLElement 或其子类型的实例。下表列出了所有 HTML 元素及其对应的类型（部分元素已经废弃）。
...
| ADDRESS                                                        | HTMLElement                                                             |
| DEL                                                            | HTMLModElement                                                          |
| <em class="hover-info" data-title="该标签已废弃">APPLET</em>   | <em class="hover-info" data-title="该标签已废弃">HTMLAppletElement</em> |
| DFN                                                            | HTMLElement                                                             |
| AREA                                                           | HTMLAreaElement                                                         |
| <em class="hover-info" data-title="该标签已废弃">DIR</em>      | HTMLDirectoryElement                                                    |
| B                                                              | HTMLElement                                                             |
| DIV                                                            | HTMLDivElement                                                          |
| BASE                                                           | HTMLBaseElement                                                         |
| DL                                                             | HTMLDListElement                                                        |
| <em class="hover-info" data-title="该标签已废弃">BASEFONT</em> | HTMLBaseFontElement                                                     |
| DT                                                             | HTMLElement                                                             |
| BDO                                                            | HTMLElement                                                             |
| EM                                                             | HTMLElement                                                             |
| BIG                                                            | HTMLElement                                                             |
...
| <em class="hover-info" data-title="该标签已废弃">FONT</em>     | <em class="hover-info" data-title="该标签已废弃">HTMLFontElement</em>   |
...
| <em class="hover-info" data-title="该标签已废弃">CENTER</em>   | HTMLElement                                                             |
...
| <em class="hover-info" data-title="该标签已废弃">S</em>        | <em class="hover-info" data-title="该标签已废弃">HTMLElement </em>      |
| HR                                                             | HTMLHRElement                                                           |
| SAMP                                                           | HTMLElement                                                             |
...
| <em class="hover-info" data-title="该标签已废弃">STRIKE</em>   | <em class="hover-info" data-title="该标签已废弃">HTMLElement</em>       |
| INS                                                            | HTMLModElement                                                          |
| STRONG                                                         | HTMLElement                                                             |
| <em class="hover-info" data-title="该标签已废弃">ISINDEX</em>  | HTMLIsIndexElement                                                      |
...
| <em class="hover-info" data-title="该标签已废弃">MENU</em>     | <em class="hover-info" data-title="该标签已废弃">HTMLMenuElement</em>   |
...
| <em class="hover-info" data-title="该标签已废弃">U</em>        | <em class="hover-info" data-title="该标签已废弃">HTMLElement</em>       |
...
这里列出的每种类型都有关联的属性和方法。本文档会涉及其中的很多类型。
...
| oldValue           | 如果在 MutationObserverInit 对象中启用（attributeOldValue 或 characterData OldValue 为 true），"attributes""characterData"的变化事件会设置这个属性为被替代的值"childList"类型的变化始终将这个属性设置为 null |
...
MutationObserver 可以观察文本节点（如 Text、Comment 或 ProcessingInstruction 节点）中字符的添加、删除和修改。要为字符数据注册回调，需要在 MutationObserverInit 对象中将 characterData 属性设置为 true，<em class="hover-info" data-title="设置元素文本内容的标准方式是textContent 属性。Element 类也定义了 innerText 属性，与 textContent 类似。但 innerText 的定义不严谨，浏览器间的实现也存在兼容性问题，因此不建议再使用了。">如下所示</em>：
...
MutationObserver 接口就是为替代这些事件而设计的更实用、性能更好的方案。将变化回调委托给微任务来执行可以保证事件同步触发，同时避免随之而来的混乱。为 MutationObserver 而实现的记录队列，可以保证即使变化事件被爆发式地触发，也不会显著地拖慢浏览器。

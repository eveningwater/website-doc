...
<dl>
    <dt>严格模式下保留:</dt>
    <dd>implements package public interface protected static let private</dd>
</dl>
...
八进制字面量在严格模式下是无效的，会导致 JavaScript 引擎抛出语法错误。<sup>①</sup>
...
对于这种问题，一个很好的思路，就是将浮点数转换成整数，相加完成之后再转换成小数，例如:

```js
const a = 0.1,b = 0.2,c = 0.3;
const toInt = (n) => {
    const l = Number(n.toString().slice(1).length);
    //可以使用**代替Math.pow,如Math.pow(10,l)等价于10 ** l
    return {
        intNum: n * Math.pow(10, l),
        digit: Math.pow(10,l) 
    };
};
//确保a和b都是小数,并且a和b的小数位一样
const d = toInt(a).intNum + toInt(b).intNum === toInt(c).intNum;
if (d) {
    console.log("you got " + toInt(c).intNum / toInt(c).digit);
}
```
...
我们再来看如下一个应用场景，当后端返回一个带有单反斜杠的字符串时，渲染在 HTML 中会出现\,而为了避免\，我们就需要将它转义一下。

```js
//将斜杠转义，因为斜杠是特殊字符，会在javascript中被忽略
let replaceChar = (v) => v.replace(/\\/g, "\\\\");
//示例字符串:xxx\xxx\xxx\xxx.xlsx
const filterStr = (v) => replaceChar(String.raw(`${v}`));
//xxx\\xxx\\xxx\\xxx.xlsx
```
...
```js
Symbol.keyFor(123); // TypeError: 123 is not a sym
```
...
这里，num3 等于 21 是因为 num1 先减 1 之后才加 num2。变量 num4 也是 21，那是因为加法使用的也是递减后的值。递增和递减的后缀版语法一样（分别是++和--），只不过要放在变量后面。后缀版与前缀版的主要区别在于，后缀版递增和递减在语句被求值后才发生。在某些情况下，这种差异没什么影响，比如：
...
有符号整数使用 32 位的前 31 位表示整数值。第 32 位表示数值的符号，如 0 表示正，1 表示负。这一位称为**符号位（sign bit）**，它的值决定了数值其余部分的格式。正值以真正的二进制格式存储，即 31 位中的每一位都代表 2 的幂。第一位（称为第 0 位）表示 2<sup>0</sup>，第二位表示 2<sup>1</sup>，依此类推。如果一个位是空的，则以 0 填充，相当于忽略不计。比如，数值 18 的二进制格式为 00000000000000000000000000010010，或更精简的 10010。后者是用到的 5 个有效位，决定了实际的值（如图 3-3 所示）。

<div class="image-container">
    <img src="./docs/javascript/images/3.png" alt="图片3-3" title="图3-3" >
...
注意在移位后，数值右端会空出 5 位。左移会以 0 填充这些空位，让结果是完整的 32 位数值（见图 3-4）。

<div class="image-container">
    <img src="./docs/javascript/images/4.png" alt="图片3-4" title="图3-4" >
...
同样，移位后就会出现空位。不过，右移后空位会出现在左侧，且在符号位之后（见图 3-5）。ECMAScript 会用符号位的值来填充这些空位，以得到完整的数值。

<div class="image-container">
    <img src="./docs/javascript/images/5.png" alt="图片3-5" title="图3-5" >
...
在对 -64 无符号右移 5 位后，结果是 134 217 726。这是因为 -64 的二进制表示是 11111111111111111111111111000000，无符号右移却将它当成正值，也就是 4294 967 232。把这个值右移 5 位后，结果是 00000111111111111111111111111110，即 134 217 726。
...
在这个例子中，变量 myObject 会被赋予两个值中的一个。其中，preferredObject 变量包含首选的值，backupObject 变量包含备用的值。如果 preferredObject 不是 null，则它的值就会赋给 myObject；如果 preferredObject 是 null，则 backupObject 的值就会赋给 myObject。这种模式在 ECMAScript 代码中经常用于变量赋值，本文档后面的代码示例中也会经常用到。
...
for-of 循环会按照可迭代对象的 next()方法产生值的顺序迭代元素。关于可迭代对象，本文档将在第 7 章详细介绍。
...
```js
label: statement //label只是标签名的一个代名词，换句话说这里的label可以换成任意的英文单词
```

...
function watchURL(){
    ...
    if(!__DEV__){
        ...
        if(!docDetail.hasOwnProperty(docKey)){
            const path = location.pathname.slice(0,location.pathname.indexOf(".html") + 5)
...
function createPopper(parent, childTag, placement, position = "fixed", modifiers = []) {
    ...
    const buttons = parent.querySelectorAll(childTag), tooltips = parent.querySelectorAll('.tooltip');
...
function requestContent(contentURL, navList) {
    ...
    requestGet("./docs/" + docDetail[docKey].content.detail + '/' + contentURL + '.md').then(res => {
        if (res.status === 200) {
            ...
            h1Element.textContent = new NumberToChinese((navList.indexOf(contentURL) + 1)).toLower() + '、' + h1Element.textContent;
            ...
            if(tableElements.length){
                tableElements.forEach(table => {
                    table.insertAdjacentHTML("afterend",`<div class="table-wrapper"><table>${ table.innerHTML }</table></div>`);
                    setTimeout(() => {
            ...
            chapterButtons.forEach(btn => on(btn,'click',(e) => {
            ...
            if(hoverInfoElements){
                hoverInfoElements.forEach(item => {
                    const hoverInfoTooltip = createElement(`<div class="hover-info-tooltip tooltip"><div class="arrow"></div>${item.getAttribute('data-title')}</div>`);
                    item.appendChild(hoverInfoTooltip);
                });
                createPopper(rightContentElement, ".hover-info", "bottom", "absolute",[
                    {
                        name:"offset",
                        options:{
                            offset:[5,5]
...
function createAnchor(data) {
    return new Promise((resolve, reject) => {
        ...
        data.forEach(item => {
            container += `
                <div class="anchor-item anchor-item-${item.level}" data-to=${item.id}>
...
function normalizeNavData(parentElement) {
    const navData = [];
    const children = toArray(parentElement.children).filter(el => el.tagName.toLowerCase().match(/h[1-6]/));
    children.forEach(el => {
        navData.push({
...
function setDocTitle(title,subTitle) {
    ...
    if(subTitle){
        ...
        headerContentElement.insertBefore(subTitleElement,titleElement.nextSibling);
    }
}
...
function setIframeHeight(elements){
    elements.forEach(item => {
        on(item,'load',() => {
            const outerHeight = item.contentWindow.document.querySelector('html').scrollHeight + 34;
...
window.onload = function () {
    ...
    requestGet("./docs/" + content.nav).then(res => { if (res.status === 200) renderNavList(res.data); });
    setLogo(icon);
    setBrowserIcon(icon);

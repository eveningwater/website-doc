const eventType = navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i) ? ['touchstart', 'touchmove', 'touchend'] : ['mousedown', 'mousemove', 'mouseup'];
...
const easeInOutCubic = (value) => value < 0.5 ? cubic(value * 2, 3) / 2 : 1 - cubic((1 - value) * 2, 3) / 2;
const scrollTop = (element, top, time = 500) => {
    const beginTime = Date.now();
    const begin = 0, end = top;
